...
fun main() {
    ...
    println("part1=" + runProgram(line.toMutableList(), BigInteger.ONE))
    println("part2=" + runProgram(line.toMutableList(), BigInteger.TWO))
}

private fun runProgram(nums: MutableList<BigInteger>, initialInput: BigInteger, curIndex: Int = 0): BigInteger {
    val input = mutableListOf(initialInput)
    ...
    while (nums[index] != 99.toBigInteger()) {
        when (nums[index].toString().last()) {
            ...
            '3' -> index = opcodeModifySimple(nums, index, input.removeAt(0), relativeBase)
            '4' -> { output.add(param(1)); index += 2 }
