...
fun main() {
    ...
    println("part1="+navigate(coords.take(1024).toSet(), start, finish, 0))
    ...
    while (max - min > 1) {
        ...
        cache.clear()
        val nextResult = navigate(coords.take(next).toSet(), start, finish, 0)
...
private fun navigate(obstacles: Set<Coord>, cur: Coord, end: Coord, moves: Int): Int? {
    cache[cur]?.let { if (it <= moves) return null }
    cache[cur] = moves
    if (cur == end) return moves
    return cur.around().filter { it !in obstacles && listOf(it.col, it.row).all { x -> x in 0..end.col } }
        .mapNotNull { navigate(obstacles, it, end, moves + 1) }.minOrNull()
}
